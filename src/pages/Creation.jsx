        ...
        {Object.values(CLASSES).map(cls => {
          ...
          return (
            <div
              ...
              onMouseEnter={e => {
                e.currentTarget.style.borderColor = cls.couleur
                e.currentTarget.style.transform = 'translateY(-2px)'
                e.currentTarget.style.boxShadow = `0 8px 32px ${cls.couleur}22`
              }}
              onMouseLeave={e => {
            ...
            >
              ...
              <div style={{
                ...
                background: count > 0 ? cls.couleur + '33' : 'var(--bg3)',
                border: `1px solid ${count > 0 ? cls.couleur : 'var(--border)'}`,
                borderRadius: '999px',
                padding: '0.1rem 0.6rem',
                fontSize: '0.75rem',
                color: count > 0 ? cls.couleur : 'var(--muted)',
              }}>
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <input
          ...
          style={{
            ...
            color: 'var(--text)', fontSize: '1.1rem', width: '100%',
          }}
        />
        <button
          onClick={() => nom.trim() && onSuivant(nom.trim())}
          disabled={!nom.trim()}
          style={{
            background: nom.trim() ? cls.couleur : 'var(--bg3)',
            color: nom.trim() ? '#000' : 'var(--muted)',
            fontFamily: 'var(--font-title)', fontSize: '0.9rem',
            padding: '0.9rem', borderRadius: 'var(--radius)',
            fontWeight: 700, letterSpacing: '0.05em',
            transition: 'all 0.2s', cursor: nom.trim() ? 'pointer' : 'default',
          }}
        >
          Répartir les points →
        </button>
        <button onClick={onRetour} style={{ background: 'transparent', color: 'var(--muted)', fontSize: '0.9rem' }}>
          ← Changer de race
        </button>
      </div>
    </div>
...
function RepartitionStats({ classe, nom, onCreer, onRetour }) {
  const cls = CLASSES[classe]
  ...
  function changerStat(key, valeur) {
    ...
    // Vérifier qu'on ne dépasse pas le total
  ...
  async function handleCreer() {
    ...
    setSaving(true)
    await onCreer({ nom, classe, stats, pv })
    setSaving(false)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at 50% 0%, #1e1030 0%, #0d0b0f 70%)', padding: '1.5rem', maxWidth: '540px', margin: '0 auto' }}>
      ...
      {/* Compteur de points */}
      <div style={{
        background: restants === 0 ? 'rgba(92,224,168,0.1)' : restants < 0 ? 'rgba(224,92,92,0.1)' : 'var(--bg2)',
        border: `1px solid ${restants === 0 ? 'var(--success)' : restants < 0 ? 'var(--danger)' : 'var(--border)'}`,
        ...
        <div style={{
          ...
          color: restants === 0 ? 'var(--success)' : restants < 0 ? 'var(--danger)' : 'var(--text)',
        }}>
          {restants}
        </div>
        <div style={{ fontSize: '0.75rem', color: 'var(--muted)', marginTop: '0.3rem' }}>
          {totalAlloue} / {TOTAL_POINTS} attribués
        </div>
      </div>
            ...
            <div key={key} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <button
                    onClick={() => changerStat(key, val - 1)}
                    disabled={val <= min}
                    style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: val <= min ? 'var(--border)' : 'var(--text)', borderRadius: '4px', width: '26px', height: '26px', fontSize: '1rem', cursor: val <= min ? 'default' : 'pointer' }}
                  >−</button>
                  <span style={{ fontWeight: 700, fontSize: '1.2rem', color: cls.couleur, minWidth: '2rem', textAlign: 'center' }}>{val}</span>
                  <button
                    onClick={() => changerStat(key, val + 1)}
                    disabled={val >= max || restants <= 0}
                    style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: (val >= max || restants <= 0) ? 'var(--border)' : 'var(--text)', borderRadius: '4px', width: '26px', height: '26px', fontSize: '1rem', cursor: (val >= max || restants <= 0) ? 'default' : 'pointer' }}
                  >+</button>
                </div>
              </div>

              {/* Slider 0–40 avec zone autorisée */}
              <div style={{ position: 'relative', padding: '0.3rem 0' }}>
                ...
                {/* Zone autorisée (min→max) en couleur atténuée */}
                ...
                {/* Valeur actuelle (0→val) en couleur pleine */}
                ...
                <input
                  type="range"
                  min={0} max={SLIDER_MAX} value={val}
                  onChange={e => changerStat(key, parseInt(e.target.value))}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
        <button
          onClick={handleCreer}
          disabled={restants !== 0 || saving}
          style={{
            background: restants === 0 ? 'var(--gold)' : 'var(--bg3)',
            color: restants === 0 ? '#000' : 'var(--muted)',
            fontFamily: 'var(--font-title)', fontSize: '0.95rem',
            padding: '1rem', borderRadius: 'var(--radius)',
            fontWeight: 700, letterSpacing: '0.05em',
            cursor: restants === 0 ? 'pointer' : 'default',
            transition: 'all 0.2s',
          }}
        >
...
export default function Creation() {
  ...
  async function creerPersonnage({ nom, classe, stats, pv }) {
    const nouveau = {
      nom,
      classe,
      pv_max: pv,
  ...
  if (etape === 'nom')    return <SaisieNom classe={classe} onSuivant={n => { setNom(n); setEtape('stats') }} onRetour={() => setEtape('classe')} />
  if (etape === 'stats')  return <RepartitionStats classe={classe} nom={nom} onCreer={creerPersonnage} onRetour={() => setEtape('nom')} />
}